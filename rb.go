...
// ============================================================
// HATE FILTER - ФИЛЬТР НЕНАВИСТИ (УЛУЧШЕННЫЙ С МОРФОЛОГИЕЙ)
// ============================================================
...
// Генерация морфологических вариантов (расширена уменьшительными суффиксами)
func (hf *HateFilter) generateMorphVariants(word string) []string {
	...
	// Русские падежи и склонения (добавлены уменьшительные суффиксы)
	ruSuffixes := []string{
		...
		// НОВЫЕ уменьшительные и пренебрежительные
		"ик", "ики", "ок", "очек", "ечек", "ичек",
		"улька", "улечка", "ушечка", "ишечка",
		"яра", "яры", "юга", "юги",
	}

	// Английские окончания (добавлены уменьшительные)
	enSuffixes := []string{
		"", "s", "es", "ed", "ing", "er", "est",
	...
	deSuffixes := []string{
		...
		"nis", "sal", "ling", "lein", "chen",
	}

	// Итальянские
	itSuffixes := []string{
		"", "a", "e", "i", "o", "ai", "ei", "oi",
		"are", "ere", "ire", "ando", "endo", "endo",
		"ato", "uto", "ito", "zione", "sione", "mento",
	}
	...
	ptSuffixes := []string{
		...
		"ado", "ido", "ção", "são", "mento", "dade",
	}

	// Голландские
	nlSuffixes := []string{
		"", "en", "s", "e", "er", "st", "t",
		"heid", "nis", "schap", "ing", "lijk",
	}

	// Польские
	plSuffixes := []string{
		"", "a", "e", "i", "ie", "u", "ów", "om", "ami", "ach",
		"ek", "ka", "ku", "kiem", "ce",
		"czyk", "czka", "czku", "czkiem",
		"ski", "ska", "skie", "skiem",
	}

	// Украинские
	ukSuffixes := []string{
		"", "а", "у", "ом", "і", "ою", "и", "ів", "ам", "ами", "ах",
		"ка", "ку", "кою", "ці", "к",
		"ень", "еня", "еню", "енем", "ені",
	}
	...
	// Турецкие
	trSuffixes := []string{
		"", "ler", "lar", "de", "da", "den", "dan",
		"e", "a", "i", "ı", "u", "ü",
		"im", "in", "sin", "iz", "siniz", "ler",
	}
	...
	// Комбинируем все суффиксы
	allSuffixes := append(ruSuffixes, enSuffixes...)
	allSuffixes = append(allSuffixes, esSuffixes...)
	allSuffixes = append(allSuffixes, frSuffixes...)
	allSuffixes = append(allSuffixes, deSuffixes...)
	allSuffixes = append(allSuffixes, itSuffixes...)
	allSuffixes = append(allSuffixes, ptSuffixes...)
	allSuffixes = append(allSuffixes, nlSuffixes...)
	allSuffixes = append(allSuffixes, plSuffixes...)
	allSuffixes = append(allSuffixes, ukSuffixes...)
	allSuffixes = append(allSuffixes, arSuffixes...)
	allSuffixes = append(allSuffixes, heSuffixes...)
	allSuffixes = append(allSuffixes, trSuffixes...)
	allSuffixes = append(allSuffixes, jaSuffixes...)
	allSuffixes = append(allSuffixes, zhSuffixes...)
	allSuffixes = append(allSuffixes, koSuffixes...)
	allSuffixes = append(allSuffixes, hiSuffixes...)
...
// Генерация вариантов с учетом регистра
func (hf *HateFilter) generateCaseVariants(word string) []string {
	variants := []string{word}
		...
		for j, r := range runes {
			if j == i {
				if unicode.IsUpper(r) {
					newWord.WriteRune(unicode.ToLower(r))
				} else {
					newWord.WriteRune(unicode.ToUpper(r))
				}
			} else {
				newWord.WriteRune(r)
			}
		}
		variants = append(variants, newWord.String())
	}
...
func (hf *HateFilter) initSlurs() {
	...
	hf.slurs["ru"] = []string{
		...
		// НОВЫЕ СЛОВА ДЛЯ РАСПОЗНАВАНИЯ "ТРАНСЫ"
	...
	// Остальные языки (без изменений, но они есть)
	// SPANISH
	hf.slurs["es"] = []string{
...
if strings.EqualFold(strings.TrimSpace(text), "лгбт") || strings.EqualFold(strings.TrimSpace(text), "lgbt") {
	...
	// Составные фразы (добавлены новые)
	phrases := []string{
		"мужик в юбке", "баба с яйцами", "женщина с членом",
		"lgbt agenda", "gender ideology", "rainbow mafia",
		"ideología de género", "teoría de género", "genderideologie",
		"genderwahn", "ideologia gender", "ideologia de gênero",
		"cinsiyet ideolojisi", "性同一性障害", "同性恋宣传",
		// НОВЫЕ
...
func (i *Interpreter) showHelp(country string) {
	...
	fmt.Fprintf(output, "  • queer <имя> = <число> - объявление переменной с плавающей точкой\n")
	...
	fmt.Fprintf(output, "  • msg <заголовок> <текст> [тип] [иконка] - диалоговое окно (Windows)\n")
	fmt.Fprintf(output, "  • antiHomoPhobe [секунды] - антигомофобная защита\n")
	fmt.Fprintf(output, "  • lgbtImg <промпт> <ширина> <высота> - генерация изображения\n")
	fmt.Fprintf(output, "  • getLGBTResources [страна] [тип] [город] [услуга] - поиск ресурсов\n")
	fmt.Fprintf(output, "  • getDailyAffirmation [тема] - аффирмация дня\n")
	fmt.Fprintf(output, "  • defineTerm <термин> [язык] - определение термина\n")
}
...
func (i *Interpreter) runOrientationDemo() {
	...
	fmt.Fprintf(output, "🎨 Генерация ЛГБТ-изображений\n")
	fmt.Fprintf(output, "🌈 Функции поддержки и защиты\n")
}
...
func runExample() {
	program := `
@ Пример LGBTScript CLI v8.0 с улучшенным фильтром

RAINBOW main() {
    COMINGOUT "🏳️‍🌈 LGBTScript CLI Demo v8.0";
    COMINGOUT "🛡️ С улучшенным морфологическим фильтром (распознаёт 'трансы', 'трансики')";
    COMINGOUT "";
    ...
    GENDER colors = ["красный", "синий", "зеленый"];
    
    COMINGOUT "👤 Имя: " + name;
    COMINGOUT "📅 Возраст: " + age;
    COMINGOUT "📏 Рост: " + height;
    COMINGOUT "✅ Активен: " + isActive;
    COMINGOUT "🎨 Цвета: " + colors;
    
    @ Условный оператор
    CIS (age >= 18) {
        COMINGOUT "✅ Совершеннолетний";
    } CIS {
        COMINGOUT "❌ Несовершеннолетний";
    }
    
    @ Цикл
    GAY i = 0;
    PRIDE (i < 3) {
        COMINGOUT "Цикл pride: " + i;
        i = i + 1;
    }
    
    @ Проверка фильтра ненависти (теперь блокирует "трансы", "трансики" и их формы)
    COMINGOUT "";
    COMINGOUT "🛡️ Тест фильтра ненависти:";
    LESBIAN testMessage = "Эти трансы опять собрались на митинг"; // будет заблокировано
    COMINGOUT testMessage;
    
    @ Вызов msg
    msg("Привет!", "Это LGBTScript с морфологическим фильтром");
}

main();
`

	fmt.Fprintln(output, "=== LGBTScript CLI v8.0 ===")
	fmt.Fprintln(output)
...
func main() {
	...
	fmt.Fprintln(output, "🛡️ С улучшенным морфологическим фильтром ненависти (распознаёт 'трансы', 'трансики' и др.)")
	...
	fmt.Fprintln(output, "📦 Поддерживаемые типы: lesbian (string), gay (int), queer (float), nonbinary (bool), gender (array)")
	runExample()
}